...
def search_market(industry: str) -> str:
    """業種ごとにインドネシア・東南アジア・ラオスの市場をWeb検索"""
    queries = [
        f"{industry} インドネシア 市場規模 2025",
        f"{industry} 東南アジア 需要 トレンド",
...
def collect_candidates() -> str:
    """九州（福岡メイン）海外進出関連クエリで収集"""
    web_queries = [
        # 福岡メイン
        "福岡 中小企業 東南アジア 進出 検討",
        "福岡 製造業 海外展開 インドネシア ベトナム",
        "福岡 食品 伝統工芸 海外販路 拡大",
        "福岡 中小企業 輸出 アジア 新規事業",
        "福岡 ものづくり企業 海外進出 補助金",
        "site:wantedly.com 福岡 海外事業 アジア展開",
        # 九州全体
        "九州 中小企業 東南アジア 海外進出",
        "熊本 鹿児島 長崎 佐賀 大分 宮崎 製造業 海外展開",
        "九州 食品メーカー 海外輸出 アジア",
        "九州 企業 インドネシア ベトナム ビジネス",
        "site:wantedly.com 九州 グローバル 東南アジア",
        "九州 ジェトロ 海外進出支援 企業",
    ]
    news_queries = [
        "福岡 中小企業 海外進出 2025",
...
以下の検索結果から「九州（特に福岡）の中小企業で海外進出・東南アジア展開に興味がありそうな企業」を抽出・整理してください。
...
def research_market(company_name: str, industry: str) -> dict:
    """Web検索＋Claudeの知識で3カ国市場ポテンシャルを評価"""
    # Web検索で最新情報を取得
    market_data = search_market(industry)

    prompt = f"""
あなたは東南アジア市場の専門アナリストです。
以下の企業・業種について、インドネシア・東南アジア全体・ラオスそれぞれの市場ポテンシャルを評価してください。
...
{{
  ...
  "southeast_asia": {{
    "score": スコア（1〜10）,
    "summary": "評価コメント（ベトナム・タイ・マレーシア等を含む東南アジア全体）",
    "opportunity": "具体的なビジネスチャンス",
    "risk": "主なリスク"
  }},
  "laos": {{
    "score": スコア（1〜10）,
    "summary": "評価コメント（人口・経済成長・競合の少なさ等を含む）",
    "opportunity": "具体的なビジネスチャンス",
    "risk": "主なリスク"
  }}
}}
...
def main():
    ...
    print("⑤ 各社の市場調査中（インドネシア・東南アジア・ラオス）...")
    market_data = {}
